...
func TestAccountCommands(t *testing.T) {
	...
	_, err := generateKeypair(keypath, testPassword)
	if err != nil {
		t.Fatal(err)
	}
	...
	testcases := []struct {
		...
		{
			"Test cross account generate --password \"abc\"",
			[]string{"password"},
			[]interface{}{"abc"},
			handleGenerateCmd,
		},
		...
		{
			"Test cross account import --password \"abc\" --privateKey 000000000000000000000000000000000000000000000000000000416c696365",
			[]string{"password", "privateKey"},
			[]interface{}{"abc", "000000000000000000000000000000000000000000000000000000416c696365"},
			handleImportCmd,
		},

		{
			"Test cross account list",
			[]string{},
			[]interface{}{},
			handleListCmd,
		},
...
func TestImportKey(t *testing.T) {
	...
	importkeyfile, err := generateKeypair(keypath, testPassword)
	if err != nil {
		t.Fatal(err)
	}
