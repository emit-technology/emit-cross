...
import (
	...
	ethtest "github.com/emit-technology/emit-cross/shared/ethereum/testing"
...
func createTestWriter(t *testing.T, cfg *Config, errs chan<- error) (*writer, func()) {
	...
	erc20Handler, err := ERC20Handler.NewERC20Handler(cfg.erc20HandlerContract, conn.Client())
	if err != nil {
		t.Fatal(err)
	}

	writer.setContract(bridge, erc20Handler)
...
func TestDuplicateMessage(t *testing.T) {
	...
	dataHash := ConstructErc20ProposalDataHash(contracts.ERC20HandlerAddress, m.Payload[0].([]byte), m.Payload[1].([]byte))
...
func TestCallData(t *testing.T) {
	receipt := common.HexToAddress("6436b2756cda97f28321c665a951ba3ad8a896a5")

	data := utils.ConstructErc20DepositData(receipt.Bytes(), big.NewInt(1000000000000000000))
